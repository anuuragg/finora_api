...
const addIncomeSrc = async (req, res) => {
  try {
    ...
    if (!userId) return res.status(404).json({ error: "User not found!" });
    if (!incomeSrc)
      return res.status(404).json({ error: "No income source was entered" });
    ...
    if (!income_src)
      return res
        .status(500)
        .json({ error: "Unable to create the income source!" });
    res.status(201).json({ income_src });
  } catch (err) {
...
const updateIncomeSrc = async (req, res) => {
  try {
    ...
    if (!userId) return res.status(404).json({ error: "User not found!" });
    if (!old_income_src)
      return res.status(404).json({ error: "Old income source missing" });
    if (!new_income_src)
      return res
        .status(404)
        .json({ error: "No new income source was entered" });
    ...
    if (!updatedIncomeSrc)
      return res
        .status(500)
        .json({ error: "Unable to update the income source!" });
...
const deleteIncomeSrc = async (req, res) => {
  try {
    await dbConnect();
    const deleted_src = await IncomeSrc.findOneAndDelete({
      income_src: req.params.income_src,
    });
    if (!deleted_src)
      return res.status(500).json({ error: "Income source not found!" });
